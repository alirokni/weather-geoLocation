...
var myGeoLocationApp = myGeoLocationApp || (function () {
  ...
  var _searchGetLocation = function (cityName, $latitude, $longitude) {
    $('#js-location-search')
      .val('');
    $('.js-forecast-container-report')
      .html('');
    $('#js-container-message')
      .text('Loading ...')
      .addClass('container-message bg-info');
    ...
    $.ajax({
      ...
      data: _accessLocation(cityName, $latitude, $longitude),
      success: _weatherForecastReport,
      error: function (textStatus, error) {
        var err = textStatus + ', ' + error;
        console.log('Request Failed: ' + err);
      }
  ...
  var _accessLocation = function (cityName, $latitude, $longitude) {
    var weatherParam;
    if (cityName) {
  ...
  var _weatherReport = function (data) {
    var weather = data,
    ...
    if (weather.cod !== '404') {
      $('#js-container-message')
        .text('')
        .removeClass('container-message bg-danger');
      $('.js-today-report')
        .text(showUpdatedDate);
      $('#js-city')
        .text(weather.name);
      $('#js-country')
      ...
      $('#js-weather-main')
        .text();
      $('#js-weather-temp-F')
        .html(showUpdatedFahrenheitTemp + " <span class='js-f-d f-d mute'>&deg;F</span>");
      $('#js-weather-temp-C')
        .html(showUpdatedCelsiusTemp + " <span class='js-c-d c-d'>&deg;C</span>");
      $('#js-img-icon')
        .prop('src', 'http://openweathermap.org/img/w/' + weather.weather[0].icon + '.png');
      $('#js-container-report')
        .removeClass('hidden');
    } else {
      $('#js-container-message')
        .text(weather.message)
        .addClass('container-message bg-danger');
  ...
  var _getDisplayedDegree = function () {
      $('.js-c-d.c-d')
        .on('click', function () {
          if ($('.js-f-d.f-d')
            .hasClass('mute')) {
  ...
  var _weatherForecastReport = function (data) {
    var weather = data,
  ...
  var findMyLocationWeather = function () {
    // check if browser supports geo location and get coordination, else call search by city name function
    if (!navigator.geolocation) {
      $('#js-container-message')
        .text('Your browser is not supporting html5 geo location.\nA Please use Search for your location.')
        .addClass('container-message bg-danger');
    } else {
      ...
      $('#js-container-message')
        .text('Loading ...')
        .addClass('container-message bg-info');
      $('.information .js-geolocation-note')
        .addClass('bg-warning');
  ...
  var _findLocationCoordination = function ($latitude, $longitude) {
  ...
  var searchLocationEnter = function () {
    var $serachVal = $('#js-location-search')
      .val();
    $('#js-location-list')
      .addClass('hidden');
    if (!!$serachVal.length) {
  ...
  */
  ...
  var _DateConversion = function (timeStamp) {
  ...
  var _drawTemperatureChart = function (weather) {
    var dateNuObj = [],
      faTempArr = [];

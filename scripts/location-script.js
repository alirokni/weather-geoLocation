...
var myGeoLocationApp = myGeoLocationApp || (function () {
    ...
    _getUrl = function(n) {
      var appId = '4d3aaef3c8ef1cc9ffe7bf1c91665949',
    ...
    _weatherReport = function(data) {
      ...
      if (weather.cod !== '404') {
        ...
        $('#temp-F').text(showUpdatedFahrenheitTemp);
        $('#temp-C').text(showUpdatedCelsiusTemp);
    ...
    _geoSuccess = function(position) {
      ...
      $latitude = typeof(position.coords) == "undefined" ? position.latitude : position.coords.latitude.toFixed(3);
      $longitude = typeof(position.coords) == "undefined" ? position.longitude : position.coords.longitude.toFixed(3);
      $('#js-lat').text($latitude);
      $('#js-lon').text($longitude);
      $('#js-acc').text(typeof(position.coords) == "undefined" ? "Somewhat" : position.coords.accuracy); // position.coords.accuracy
    ...
    _geoFail = function(error) { //  geoError = { 1: 'PERMISSION DENIED', 2: 'POSITION UNAVAILABLE', 3: 'TIMEOUT', 4: http  };
      var errorMessage = '';
      if (error.code === 1) {
        ...
        $('.information .js-geolocation-note').removeClass('bg-warning');
        errorMessage = 'Only secure origins are allowed. Displaying Weather Report using IP Address instaed of GeoLocation due to use of HTTP protocol.';
        $('#js-container-message').text(errorMessage).addClass('container-message bg-danger');
        $('.information .js-ipinfo-note').addClass('bg-warning');
        var setMessageTimerFail = new SetMessageTimer('.information .js-ipinfo-note', 'bg-warning', 3000);
        setMessageTimerFail.showMessage();
        // using http://ipinfo.io/ in case geoLocation is disabled
        _getIPAddress();
      } else {
